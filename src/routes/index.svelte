<script lang="ts">
    ...
    const success = (position: { coords: { latitude: any; longitude: any; }; }) => {
        ...
        positionerror = false;
        goto(`forecast/${latitude.toFixed(2)}/${longitude.toFixed(2)}`)
    }
...
<div class="flex justify-center w-full flex-col items-center">
    <div class="m-8 flex flex-col w-full justify-center items-center">
        <div class="flex">
            ...
            <button class="btn btn-circle btn-outline ml-4 flex items-center justify-center" on:click={getLocation}>
                {#if positionwaiting}
                    <svg
        ...
        <div class="overflow-x-auto w-full m-8 lg:w-1/2">
            {#await towns}
                <table class="table table-compact w-full"></table>
            {:then towns}
                ...
                <table class="table table-compact w-full">
                    ...
                    <tbody>
                    {#each towns.results as item, index}
                        <tr class="hover:active"
                            on:click={navigate(`forecast/${item.id}`)}>
                            <th>{ index + 1 }</th>
                            <td>
                                { item.name }
                            </td>
